...
if ( ! class_exists( 'context_blog_Control_Repeater' ) ) {
	/**
	 * Repeater control
	 */
	class context_blog_Control_Repeater extends WP_Customize_Control {
		...
		public function __construct( $manager, $id, $args = array() ) {
			...
			$this->row_label = array(
				'type'  => 'text',
				'value' => esc_attr__( 'row', 'context-blog' ),
				'field' => false,
			);
			...
			if ( empty( $this->button_label ) ) {
				...
				$this->button_label = sprintf( esc_attr__( 'Add new %s', 'context-blog' ), $this->row_label['value'] );
			}
				...
				foreach ( $this->fields as $field ) {
					if ( isset( $field['type'] ) ) {
						if ( 'color' === $field['type'] ) {
							...
							wp_enqueue_style( 'font-awesome-51', get_template_directory_uri() . '/assets/css/font-awesome5.css', array(), '5.1.12' );

						}
					}
				}
			}
			wp_enqueue_script( 'context-blog', get_template_directory_uri() . '/assets/customizer/repeater.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-sortable' ), false, true );
			wp_enqueue_style( 'context-blog', get_template_directory_uri() . '/assets/customizer/repeater.css', null );
		}
		...
		protected function render_content() {
			...
			<?php if ( isset( $this->choices['limit'] ) ) : ?>
				...
				printf( esc_attr__( 'Limit: %s rows', 'context-blog' ), esc_html( $this->choices['limit'] ) );
				?>
				</p>
			<?php endif; ?>
		...
		public function repeater_js_template() {
			...
			<label>
				...
				<span class="description customize-control-description">{{{ field.description }}}</span>
				<# } #>
				<input class="color-picker-hex" type="text" maxlength="7" placeholder="<?php esc_attr_e( 'Hex Value', 'context-blog' ); ?>"  value="{{{ field.default }}}" data-field="{{{ field.id }}}" {{ defaultValue }} />
			...
			<figure class="kirki-image-attachment" data-placeholder="<?php esc_attr_e( 'No Image Selected', 'context-blog' ); ?>" >
				<# if ( field.default ) { #>
				<# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
				<img src="{{{ defaultImageURL }}}">
				<# } else { #>
				<?php esc_attr_e( 'No Image Selected', 'context-blog' ); ?>
				<# } #>
			</figure>
			
			<div class="actions">
				<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_html_e( 'Remove', 'context-blog' ); ?></button>
				<button type="button" class="button upload-button" data-label=" <?php esc_attr_e( 'Add Image', 'context-blog' ); ?>" data-alt-label="<?php esc_attr_e( 'Change Image', 'context-blog' ); ?>" >
					<# if ( field.default ) { #>
					<?php esc_attr_e( 'Change Image', 'context-blog' ); ?>
					<# } else { #>
					<?php esc_attr_e( 'Add Image', 'context-blog' ); ?>
					<# } #>
				</button>
				<# if ( field.default.id ) { #>
			...
			<figure class="kirki-file-attachment" data-placeholder="<?php esc_attr_e( 'No File Selected', 'context-blog' ); ?>" >
				<# if ( field.default ) { #>
				...
				<# } else { #>
				<?php esc_attr_e( 'No File Selected', 'context-blog' ); ?>
				<# } #>
			</figure>
			
			<div class="actions">
				<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"></button>
				<button type="button" class="button upload-button" data-label="<?php esc_attr_e( 'Add File', 'context-blog' ); ?>" data-alt-label="<?php esc_attr_e( 'Change File', 'context-blog' ); ?>" >
					<# if ( field.default ) { #>
					<?php esc_attr_e( 'Change File', 'context-blog' ); ?>
					<# } else { #>
					<?php esc_attr_e( 'Add File', 'context-blog' ); ?>
					<# } #>
				</button>
				<# if ( field.default.id ) { #>
				<input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
		...
		<# }); #>
		<button type="button" class="button-link repeater-row-remove"><?php esc_html_e( 'Remove', 'context-blog' ); ?></button>
	</div>
</li>
</script>
			<?php
		...
		function get_icon_list() {
			...
			$icons4 = include wp_normalize_path( get_template_directory() . '/inc/fontawesome5.php' );
			if ( $icons4 ) {
				?>
				<div class="font-awesome-list-template">
					<div class="font-awesome-list">
						<ul class="font-group">
							...
							<?php
							foreach ( $icons4 as $font4 => $fontval ) {
								echo '<li><i class="' . esc_attr( $font4 ) . '"></i></li>';
							}
							?>
						</ul>
					</div>
